...
We decided to look at just how many location updates and co-occurrences happened inside the Sony perimeter. Figure \ref{fig:dist_locs_sony} shows the number of location updates which we found to be inside the Sony perimeters, as we can see approximately a fifth of location updates have taken place inside the Sony perimeters. This is not surprising as we assume they spend a significant portion of their day working.
\begin{figure}[H]
    \hspace*{-1.0cm}
    \centering
    \includegraphics[scale=0.10]{stack_bar_loc_updates}
    \caption{Distribution of location updates inside and outside Sony perimeters}
    \label{fig:dist_locs_sony}
\end{figure}

 Figure \ref{fig:dist_coocs_sony} shows the number of co-occurrences inside and outside Sony perimeters, here we see a massive amount of co-occurrences happening inside the Sony perimeters. This can pose a problem as we will only be looking at co-occurrences between users that is outside Sony perimeters.
...
The number of unique co-occurrences in terms of spatial bins between two users \textit{i} and \texit{j}. Feature ID:\textit{3}.
...
The number of common travels between two users. Common travels are defined when user \textit{i} and user \textit{j} co-occur in consecutive time-bins with a different spatial bin than the previous co-occurrence. Feature ID:\textit{6}.
