...
\item Test data
\begin{enumerate}
\item without user-filtering (\textbf{Modelpair: TP-1})
\item with user-filtering using cross-occupancy of 40\% (\textbf{Modelpair: TP-2})
\end{enumerate}
\item Production data
\begin{enumerate}
\item without user-filtering (\textbf{Modelpair: PP-1})
\item with user-filtering using cross-occupancy of 10\% (\textbf{Modelpair: PP-1})
\end{enumerate}
\end{enumerate}
...
First we trained a Logistic Regression classifier which serves as our baseline classifier to compare our other model with. We trained it only with the feature two_unique_coocs. Next we trained a Random Forest classifier with all of our features.
